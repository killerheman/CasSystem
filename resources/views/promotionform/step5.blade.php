        ...
        <div class="row mt-3 ">
            <div class="col">
                ...
                <select class="form-select form-select-lg mb-3"
                    ...
                    <option value="1" {{!empty($user->step5[0])?($user->step5[0]->type=='1'?'selected':''):'' }}>ORIENTATION/REFRESHER COURSE OR ANY OTHER TRAINING PROGRAM/SUMMER SCHOOL /
                        WORKSHOPS/ QIP/ FIP ETC
                        ATTENDED: (Attach certificates)
                    </option>
                    <option value="2"  {{!empty($user->step5[0])?($user->step5[0]->type=='2'?'selected':''):'' }}> ORIENTATION/REFRESHER COURSE ATTENDED (ONLY UGC
                        RECOGNIZED):(Attach certificates)</option>
                </select>
            </div>
            <div class="col">
                <label class="fieldlabels">Place:</label> <input type="text"
                    name="place[]" placeholder="Place from" value=" {{!empty($user->step5[0])?($user->step5[0]->place):'' }}" />
            </div>
            <div class="col">
                <label class="fieldlabels">Particulars:</label> <input type="text"
                    name="particulars[]" placeholder="Particulars from" value="{{!empty($user->step5[0])?($user->step5[0]->particulars):'' }}" />
            </div>
            <div class="col">
                <label class="fieldlabels">Duration:</label> <input type="text"
                    name="duration_from[]" placeholder="Duration from" value="{{!empty($user->step5[0])?($user->step5[0]->duration_from):'' }}"/>
            </div>
            <div class="col">
        ...
        <div class="row mt-2">
            <div class="col-3">
                ...
                <input type="text" name="sponsoring_agency[]"
                    placeholder="Sponsoring Agency" value="{{!empty($user->step5[0])?($user->step5[0]->sponsoring_agency):'' }}"/>
            </div>
            <div class="col-3">
                ...
                <div class="parent4 ">
                    <div class="row mt-3 ">
                        <div class="col">
                            ...
                            <select class="form-select form-select-lg mb-3"
                                aria-label=".form-select-lg example" name="type[]">
                                <option selected value="" disabled selected>Open this select menu</option>
                                <option value="1" {{!empty($user->step5[$j])?($user->step5[$j]->type=='1'?'selected':''):'' }}>ORIENTATION/REFRESHER COURSE OR ANY OTHER TRAINING PROGRAM/SUMMER SCHOOL /
                                    WORKSHOPS/ QIP/ FIP ETC
                                    ATTENDED: (Attach certificates)
                                </option>
                                <option value="2"  {{!empty($user->step5[$j])?($user->step5[$j]->type=='2'?'selected':''):'' }}> ORIENTATION/REFRESHER COURSE ATTENDED (ONLY UGC
                                    RECOGNIZED):(Attach certificates)</option>
                            </select>
                        </div>
                        <div class="col">
                            <label class="fieldlabels">Place:</label> <input type="text"
                                name="place[]" placeholder="Place from" value=" {{!empty($user->step5[$j])?($user->step5[$j]->place):'' }}" />
                        </div>
                        <div class="col">
                            <label class="fieldlabels">Particulars:</label> <input type="text"
                                name="particulars[]" placeholder="Particulars from" value="{{!empty($user->step5[$j])?($user->step5[$j]->particulars):'' }}" />
                        </div>
                        <div class="col">
                            <label class="fieldlabels">Duration:</label> <input type="text"
                                name="duration_from[]" placeholder="Duration from" value="{{!empty($user->step5[$j])?($user->step5[$j]->duration_from):'' }}"/>
                        </div>
                        <div class="col">
                    ...
                    <div class="row mt-2">
                        <div class="col-3">
                            <label class="fieldlabels">Sponsoring Agency</label>
                            <input type="text" name="sponsoring_agency[]"
                                placeholder="Sponsoring Agency" value="{{!empty($user->step5[$j])?($user->step5[$j]->sponsoring_agency):'' }}"/>
                        </div>
                        <div class="col-3">
                            <label class="fieldlabels">Encl. No.</label>
                            <input type="text" name="sponsoring_agency"
